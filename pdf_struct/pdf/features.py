...
def extract_features(text_boxes: List[TextBox]):
    ...
    left_margin = get_pdf_margin(clusters_l, n_pages)
    right_margin = get_pdf_margin(
        cluster_positions([b.bbox[2] for b in text_boxes], horizontal_thresh)[0][::-1],
        n_pages)
    clusters_s, mappings_s = cluster_positions(
        [b1.bbox[1] - b2.bbox[1]
         for b1, b2 in pairwise(sorted(text_boxes, key=lambda b: (b.page, -b.bbox[1], b.bbox[0])))
         if b1.page == b2.page],
        line_spacing_thresh
    )
    ...
    def line_break(tb1: TextBox, tb2: TextBox):
        if tb1 is None:
            return True
        return tb1.bbox[2] not in right_margin

    def indent(tb1: TextBox, tb2: TextBox):
        if tb1 is None or tb2 is None:
            return 3
        if mappings_l[tb1.bbox[0]] < mappings_l[tb2.bbox[0]]:
            return 1
        if mappings_l[tb1.bbox[0]] > mappings_l[tb2.bbox[0]]:
            return 2
        return 0

    def centered(tb: TextBox):
        if tb is None:
            return False
        if tb.bbox[0] in left_margin:
            return False
        right_space = right_margin.mean - tb.bbox[2]
        left_space = tb.bbox[0] - left_margin.mean
        return abs(right_space - left_space) < 20

    def extra_line_space(tb1: TextBox, tb2: TextBox):
        if tb1 is None or tb2 is None:
            return True
        ls = tb1.bbox[1] - tb2.bbox[1]
        return ls not in line_spacing

    def dict_like(tb: TextBox):
        if tb is None:
            return False
        return ':' in tb.text and tb.bbox[2] not in right_margin

    def page_like(tb: TextBox):
        if tb is None:
            return False
        return ((tb.bbox[1] < 100 or tb.bbox[3] > 700) and
                re.search('[1-9]', tb.text) is not None)

    def page_like2(tb: TextBox):
        if tb is None:
            return False
        return ((tb.bbox[1] < 100 or tb.bbox[3] > 700) and
    ...
    def page_change(tb1: TextBox, tb2: TextBox):
        if tb1 is None or tb2 is None:
            return True
    ...
    for tb1, tb2, tb3, tb4 in groupwise(text_boxes, 4):
        ...
        feat = (
            ...
            centered(tb2),
            centered(tb3),
            extra_line_space(tb1, tb2),
            extra_line_space(tb2, tb3),
            dict_like(tb2),
            dict_like(tb3),
            page_like(tb1),
            page_like(tb2),
